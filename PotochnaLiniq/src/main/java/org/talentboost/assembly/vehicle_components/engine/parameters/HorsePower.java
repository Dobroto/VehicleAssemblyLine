...
public class HorsePower implements IEngineMainCharacteristic {
	...
	/**
	 * If the given engine specifications include horse power, this class's
	 * checkAvailability will be called. Here if the input is B-100hpT,
	 * horsePowerInput will be 100, wantsTurbo will be true and engineType will be
	 * B. The kW map is called and the horse power are converted to kW. TreeMap is
	 * used because we are searching for a range of possible keys. If the kW value
	 * is between two groups of EngineSpecifications with same values, it means that
	 * we have a match. Next we are checking if the object's engine types are the
	 * same. The last if block checks whether the client wants turbo and if that
	 * specific horse power comes with turbo.
	 */
	@Override
